...
long add_multiples_for_n(const int max_multiple, const int divisor) {
  int quo = (max_multiple - 1) / divisor;
  long sum_1_quo = (0.5 * quo) * (quo + 1);
  long sum_multiples = sum_1_quo * divisor;
...
long add_multiples_s(const int max_multiple) {
  long sum_multiples = add_multiples_for_n(max_multiple, 3);
...
void project_1_s(const int max_multiple /* = 1e3 */) {
  ...
  long sum = add_multiples_s(max_multiple);
